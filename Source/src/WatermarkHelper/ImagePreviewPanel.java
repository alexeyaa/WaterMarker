    ...
    public void propertyChange(PropertyChangeEvent e) {
        ...
        if (propertyName.equals(JFileChooser.SELECTED_FILE_CHANGED_PROPERTY)) {
            ...
            if ((name != null) &&
                ...
                try {
                    bufferdImage = javax.imageio.ImageIO.read(new File(name));
                } catch (Exception ex) {
                }
                icon = new ImageIcon(bufferdImage);
                image = icon.getImage();
                scaleImage();
                repaint();
            } else if ((name != null) &&
                    name.toLowerCase().endsWith(".tif") || name.toLowerCase().endsWith(".tiff")) {
                RenderedImage images = JAI.create("fileload", name);
                BufferedImage bufferdImage = helper.convertRenderedImage(images);
                icon = new ImageIcon(bufferdImage);
                image = icon.getImage();
                scaleImage();
                repaint();
